...
export function AccountSettings({ fail }: Props) {
  ...
  const load = () => api("/accounts").then(setStatus);
  useEffect(() => {
    load().catch(fail);
  }, []);
  ...
  return (
    <div className="card settings-form">
      ...
      <p>
        ...
        encrypted on your host. No email sending permissions are requested.
      </p>
      {outcome === "connected" && (
        <p role="status">
  ...
  return (
    <fieldset disabled={busy || connecting} className="account-provider">
      ...
      <details open={!provider.secretConfigured}>
        ...
        <p>
          {name === "Google"
            ? "Enable Google Calendar API and add your account as a test user while the consent app is in testing. Google test-mode refresh grants may require periodic reconnection."
