...
| Commitments and capacities | `apps/web/src/goals.tsx`, `apps/web/src/commitment-components.tsx`, `leam_api/commitments.py`; revisions, progress and reviewed removal |
...
| Daily planning | `apps/web/src/today.tsx`, `agenda-chat.tsx`, `leam_api/agenda.py`; saved-source daily windows, local priorities and per-day Companion bindings |
| Calendar visibility | `apps/web/src/calendar-visibility.tsx`, `leam_api/calendar_visibility.py`; exact-source local hiding and undo |
| Read-only mailbox | `leam_api/email.py`; separate Google consent and encrypted bounded inbox snapshots |
| Pending coding display | `apps/web/src/coding-pending.ts`, `session-cache.ts`, `leam_api/codex_submissions.py`; exact submission identities, steering and uncertain receipts |
| Application icons/install | `apps/web/brand/`, `apps/web/public/`, `pwa-settings.tsx`; app-owned artwork and platform-dependent install controls |
